...
public class Trie {
    ...
    String result;
    private void writeThisNodeOnly(TrieNode node) throws IOException{
    	if(node == null)
    		//result+="null";
    		bw.append("n");
    	else{
    		bw.append(node.ch +",");
	    	bw.append(String.valueOf(node.webScore) + ",");
	    	bw.append(String.valueOf(node.userScore) + ",");
	    	bw.append(String.valueOf(node.isWord?1:0));
//    		result += node.ch +",";
...
//	    	result += String.valueOf(node.isWord?1:0);
    ...
    private void writeNodeAndBest3(TrieNode node) throws IOException{
...
//	    			result += ",";
    ...
    private void writeTrie(TrieNode node) throws IOException{    	
        ...
        for(Entry<Character, TrieNode> entry : children.entrySet()) {
        	...
        	if(node.ch == ' ')
        	{
        		if(val == node.best3[0] || val == node.best3[1] || val == node.best3[2]){
        			bw.append(",");
//        			result += ",";
    	        	writeTrie(val);
        		}
